...
$start = ($current_page * $games_per_page) - $games_per_page + 1;
$end = $current_page * $games_per_page;
if($end > $total_games) {
  $end = $total_games;
}
...
$i = 1; // 4 rows
$games = get_games_subset($start, $end);
...
<?= $paginator; ?>
      ...
      foreach($games as $row) {
        ...
        if(!empty($row["description_rus"])) {
          $description = $row["description_rus"];
        } else {
